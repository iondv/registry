...
module.exports = (function () {
  ...
  function formatDate(obj, format) {
    ...
    return result === 'Invalid date' ? obj : result;
  }

  function getCollectionVal(obj, key, sep = ';', format) {
    let parts = key.split('.');
    let result = obj;
    for (let i = 0; i < parts.length; i++) {
      if (Array.isArray(result) && parts[i]) {
        let tmp = [];
        result.forEach(res => tmp.push(getCollectionVal(res, parts.slice(i).join('.'), sep, format)));
        result = tmp.join(sep);
      } else if (typeof result === 'object' && result && parts[i]) {
        result = result[parts[i]];
        if (format) {
      ...
      row.findall('c').forEach(function (cell) {
        ...
        if (cell.attrib.t === 's') {
          ...
          self.extractPlaceholders(string).forEach(function (placeholder) {
            ...
            var substitution = getVal(substitutions, placeholder.name),
              newCellsInserted = 0;
            ...
            if (placeholder.full && (['table', 'table_col'].indexOf(placeholder.type) > -1) && substitution instanceof Array) {
              newCellsInserted = self.substituteTable(
                row, newTableRows,
                cells, cell,
                namedTables, substitution, placeholder
              );
  ...
  Workbook.prototype.stringify = function (value) {
    ...
    if (value instanceof Date) {
      ...
      return Number((value.getTime() - value.getTimezoneOffset() * 60000) / 86400000 + 25569).toString();
  ...
  Workbook.prototype.substituteTable = function (row, newTableRows, cells, cell, namedTables, substitution, {key, type, sep, format}) {
    var self = this,
      ...
      substitution.forEach(function (element, idx) {
        var newRow, newCell,
          ...
          value = key === '@rowNumber' ?
            (idx + 1) :
            type === 'table_col' ?
              getCollectionVal(element, key, sep, format) :
              getVal(element, key);
